...
export default function ProjectCard({ project, index }) {
  const { title, description, tags, category, slug, image, beta, openSource } = project;
  ...
  return (
    <Link
      to={`/project/${slug}`}
      className="animate-fade-up retro-card press-feedback group relative block rounded-lg border border-[#E4E7EC] bg-[#FFFFFF] transition-all duration-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#049B9F]"
      style={{
    ...
    >
      ...
      <div className="absolute -top-2.5 -left-2.5 z-10 flex flex-col items-start gap-1">
        {beta && (
          <span className="bg-[#2C2C2C] px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-[#FFFFFF]">
            Beta
          </span>
        )}
        {openSource && (
          <span className="bg-[#037B7E] px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-[#FFFFFF]">
            Open source
          </span>
        )}
      </div>
      ...
      <div className="relative overflow-hidden border-b-2 border-dashed border-[#2C2C2C]/8">
        {image && !imgError ? (
          ...
          <div className="flex h-40 w-full items-center justify-center bg-[#F5F6F8]">
      ...
      <div className="p-6">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag}
              className="bg-[#F5F6F8] px-2.5 py-0.5 text-xs font-medium uppercase tracking-wide text-[#2C2C2C]"
            >
              {tag}
            </span>
          ))}
